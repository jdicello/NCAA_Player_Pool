            ...
            for col in row.findAll("td"):
                if col.a != None:
                    if col.a.span.text.isalpha():
                        data += str(col.a.text).replace('-',',') + ','
                    else:
                        data += col.a.span.text.replace('-',',') + ','
                elif col.text != None:
...
def parseBoxScore(row):
    ...
    if int(away_id) in [team_id1,team_id2]:
        ...
        player_stats_df = player_stats_df.append(parseTeamBoxScore(soup_box, game_id, away_id, away_team, 'away', home_id, home_team))
...
#        if home_id in [team[2] for team in self.bracket]:
    if int(home_id) in [team_id1,team_id2]:
        player_stats_df = player_stats_df.append(parseTeamBoxScore(soup_box, game_id, home_id, home_team, 'home', away_id, away_team))
...
#    away_not_parsed.columns = ('game_id','team_id','opp_id','id')
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=16) as executor2:
        ...
        future_game = {executor2.submit(parseBoxScore, row): row for row in games}
